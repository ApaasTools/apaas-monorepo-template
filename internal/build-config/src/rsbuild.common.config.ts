...
/** 入口文件 */
export const entry =
	process.env.PUBLIC_ENTRY || createResolve(import.meta.url)("./src/index.ts");
/** 构建输出名 */
...
export const rsbuildCommonConfig: RsbuildConfig = {
	...
	tools: {
		...
		rspack: {
			// 规避 Rspack 模块合并（scope hoisting）在 vue SFC re-export 场景 panic：
			// "Cannot get final name for export 'render'" (concatenated_module.rs)
			optimization: {
				concatenateModules: false,
				// mangleExports: false, // 如果上面一行不够，再放开这个
			},
			resolveLoader: {
				alias: {
					// 修改内联 loader 中 worker-loader 的指向，如 `worker-loader!pdfjs-dist/es5/build/pdf.worker.js`
					"worker-loader": `${require.resolve(
		...
		postcss: {
			postcssOptions: {
				plugins: tailwindPostcssPlugins(),
			},
		},
	},
};

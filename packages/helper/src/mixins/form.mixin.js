import { widgetRequiredValidator } from "../utils";
		...
		return {
			...
			bsRefreshDebounce: debounce((newValue, oldValue) => {
				if (newValue !== oldValue) {
					...
					try {
						this.formEngine.bsEventControl.triggerEventValueChange(
							this.widget,
							event,
						);
					} catch (error) {
		...
		validatorRules() {
			...
			if (this.renderScene === "edit") {
				if (this.showRequired) {
					if (this._validate) {
						rules.push(
							this._validate("required", `${this.widget.label}是必填字段`),
						);
					}
				}
	...
	methods: {
		_validate(type, message, trigger = ["blur", "change"], isI18n = false) {
			...
			if (typeof type === "string") {
				validator.type = type;
				if (type === "required") {
					validator.validator = WidgetRequiredValidator(
						isI18n ? this.$t(message) : message,
					);
				}
